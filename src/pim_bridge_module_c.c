...
static int get_vectors_from_user(unsigned long arg, uint16_t **vector_a_out,
                                 uint16_t **vector_b_out, uint32_t *len_out) {
    struct pim_vectors descriptor;
    uint16_t *kernel_vector_a = NULL;
    uint16_t *kernel_vector_b = NULL;

    // Get the descriptor
    if (copy_from_user(&descriptor, (struct pim_vectors __user *)arg,
                       sizeof(descriptor))) {
        pr_err("PIM: Failed to copy descriptor\n");
        return -EFAULT;
    }

    if (descriptor.len1 == 0 || descriptor.len1 != descriptor.len2 ||
        descriptor.len1 > MAX_ARRAY_ELEMENTS) {
        return -EINVAL;
    }

    kernel_vector_a = kmalloc(descriptor.len1 * sizeof(uint16_t), GFP_KERNEL);
    kernel_vector_b = kmalloc(descriptor.len1 * sizeof(uint16_t), GFP_KERNEL);
    ...
    if (copy_from_user(kernel_vector_a,
                       (void __user *)descriptor.vector_a_user_addr,
                       descriptor.len1 * sizeof(uint16_t)) ||
        copy_from_user(kernel_vector_b,
                       (void __user *)descriptor.vector_b_user_addr,
                       descriptor.len1 * sizeof(uint16_t))) {
        kfree(kernel_vector_a);
        kfree(kernel_vector_b);
        return -EFAULT;
    ...
    *vector_a_out = kernel_vector_a;
    *vector_b_out = kernel_vector_b;
    *len_out = descriptor.len1;
    return 0;
}
...
static long pim_device_ioctl(struct file *file, unsigned int cmd,
    ...
    case IOCTL_VADD: {
        ...
        ret = get_vectors_from_user(arg, &kernel_vector_a, &kernel_vector_b,
                                    &len);
        if (ret) {
            return ret;
        }
